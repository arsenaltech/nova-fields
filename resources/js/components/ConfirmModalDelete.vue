...
export default {
  props:['totalFileCount','selectedFiles'],
  data: () => ({
    active: false,
    name: null,
    type: null,
  ...
  methods: {
    ...
    deleteFile() {
      let files = this.totalFileCount == 0 ? this.path : this.selectedFiles;
      return api.removeFile(files).then(result => {
        this.error = false;
        this.name = null;
        if (result == true) {
          this.$toasted.show(this.__('Deleted successfully'), { type: 'success' });
