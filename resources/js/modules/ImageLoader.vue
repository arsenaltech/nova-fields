<template>
  <transition name="fade">
    ...
    <template v-if="view == 'list'">
      ...
      <tr :loading="loading" v-bind:key="file.id" class="cursor-pointer">
        <td @click="clickStrategy('checkbox')" class="w-8">
...
export default {
  ...
  methods: {
    clickStrategy(type) {
      if(type == "name" && this.multiSelecting == false){
        return this.showInfo()
      }else if(type == "checkbox" || (type == "name" && this.multiSelecting == true)){
        return this.select()
      }
    },
